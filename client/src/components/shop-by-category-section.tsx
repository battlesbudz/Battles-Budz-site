        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="inline-flex items-center gap-2 border border-yellow-300/40 bg-yellow-300/10 px-3 py-2 text-xs font-black uppercase tracking-[0.18em] text-yellow-200">
              <ShoppingBag className="h-4 w-4" aria-hidden="true" /> Shop by category
            </p>
        ...
        >
          {shopCategories.map((category) => {
            ...
            return (
              <li
                key={category.name}
                className={`min-h-44 snap-start ${
                  isConcentrates ? "min-w-[13.25rem] sm:min-w-[15rem]" : "min-w-[11.25rem] sm:min-w-[14rem]"
                }`}
              >
                <Link
                  href={category.href}
                  className="group relative flex h-full min-h-44 flex-col justify-between overflow-hidden rounded-xl border border-yellow-300/20 bg-zinc-950 p-4 transition hover:-translate-y-1 hover:border-yellow-300/60"
                  style={{
                ...
                >
                  ...
                  <div className="relative">
                    <div className="flex justify-end">
                      <ArrowRight className="h-4 w-4 shrink-0 text-yellow-200 drop-shadow transition group-hover:translate-x-1" aria-hidden="true" />
                    </div>
                    <h3
